<?php
	...
	return sprintf("
							...
							. ( (isset($version) && isset($release) && file_exists(trim(`pwd`)."/{$this->uri}/downloads/{$this->doc}-${version}-${release}.tar.gz")) ?"<a href='{$this->uri}/downloads/{$this->doc}-".preg_replace("/%/", "%%", urlencode($version))."-${release}.tar.gz'><img class='projects_download' alt='Download {$this->doc}&#039;s latest tarball.' title='Download {$this->doc}&#039;s latest tarball.' src='{$this->uri}/graphics/tgz.png'></a>" :"<!-- download not found: " . trim(`pwd`)."/{$this->uri}/downloads/{$this->doc}-${version}-${release}.tar.gz"." -->")
							...
							. ( (isset($version) && isset($release) && file_exists(trim(`pwd`)."/{$this->uri}/downloads/{$this->doc}_${version}-${release}_amd64.deb")) ?"<a href='{$this->uri}/downloads/{$this->doc}_".preg_replace("/%/", "%%", urlencode($version))."-${release}_amd64.deb'><img class='projects_download' alt='Download {$this->doc}&#039;s latest x86_64 Debian package.' title='Download {$this->doc}&#039;s latest x86_64 Debian package.' src='{$this->uri}/graphics/x86_64.deb.png'></a>" :"<!-- download not found: " . trim(`pwd`)."/{$this->uri}/downloads/{$this->doc}_${version}-${release}_amd64.deb"." -->")

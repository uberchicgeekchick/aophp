<?php
print <<<SPECIAL
		<h1>Expressive Special</h1>
		<h2>Geek Is Chic&#039;s 24hr Podcastathon</h2>
			Its not to late: <a href='http://www.new.facebook.com/event.php?eid=27355222103'><em><strong>RSVP!</strong></em></a> &amp; remember to show up &amp; help us raise awareness, support, &amp; donations to find a cure!
		<p align='center'>
			<a href='http://geekischic.org/'>Join your sister nerd &amp; geek chicks as we work together to fight!<br/>
			keep all of our boobs: safe &amp; sound! keep our girls round &amp; around!<br/>
			<a href='http://GeekIsChic.Org/'><img src='./graphics/banners/GeekIsChic.png' width='474' height='318' border='0'/><a/><br/>
			<a href='http://GeekIsChic.Org/'>http://GeekIsChic.Org/</a><br/>
		</p><p>
			<ul>
				<lh>Join us in showing you support for a cure!</lh>
				<li>Watch &amp; chat on <a href='http://uStream.tv/iandrea'>uStream.tv/iandrea</a></li>
				<li>Or join us on IRC @ <a href='irc://irc.freenode.net/Podcastathon'>irc://irc.freenode.net/Podcastathon</a></li>
				<li>Call in, listen in, be a part of finding a cure! @ <a href='http://www.talkshoe.com/tc/1256/'>http://www.talkshoe.com/tc/12560</a></li>
				<li>There are two way to call in to share your story &amp; show your support:</li>
				<ul>
					<li>Using your favorite SIP client, like the free <a href='http://Gizmo5.Com/'>Gizmo5</a>, call <a href='sip:GeekIsChic-12560@66.212.134.192'>sip:GeekIsChic-12560@66.212.134.192</a>></li>
					<li>Or via mobile or telephone dial: <em>1-727-444-7444</em></li>
				</ul>
				<li>When asked to enter the <em>Call ID</em> enter <strong>12560</strong> and help us come even a lil closer to a cure!</li>
			</ul>
			<a href='http://www.flickr.com/groups/geekischic/'>flickr banners - where you ribbon::change your avatar</a><br/>
			To busy? No worries, just please help &amp; donate. All donations are to the Susan G. Komen Foundation<br/>
			<form action='https://www.paypal.com/cgi-bin/webscr' method='post'><input src='https://www.paypal.com/en_US/i/btn/btn_donateCC_LG.gif' type='image'><input name='cmd' value='_donations' type='hidden'><input name='business' value='contact@geekischic.org' type='hidden'><input name='item_name' value='Geek Is Chic Donates to Susan G Komen' type='hidden'><input name='no_shipping' value='0' type='hidden'><input name='return' value='http://uberchicgeekchick.com/' type='hidden'><input name='cancel_return' value='http://uberchicgeekchick.com' type='hidden'><input name='no_note' value='1' type='hidden'><input name='currency_code' value='USD' type='hidden'><input name='tax' value='0' type='hidden'><input name='lc' value='US' type='hidden'><input name='bn' value='PP-DonationsBF' type='hidden'></form>
		</p>
SPECIAL;
?>
